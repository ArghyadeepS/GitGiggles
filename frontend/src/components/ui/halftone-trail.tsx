...
export const HalftoneTrail: React.FC<HalftoneTrailProps> = ({
  ...
  useEffect(() => {
    ...
    // Ensure the first frame has valid dimensions even before ResizeObserver fires.
    const initialRect = container.getBoundingClientRect();
    if (initialRect.width > 0 && initialRect.height > 0) {
      engine.resize(initialRect.width, initialRect.height);
      const dpr = Math.min(window.devicePixelRatio, 2);
  ...
  }, [cellSize, decay, brushSize, hoverBrushSize, opacity, hoverOpacity, speedScale, hoverSelector, color]);
